...
int initialize_configuration(char* in_dsname, char* in_llq, char* out_extension, int* out_txtflag, int* out_ccsid) {
    ...
    if (get_user_configuration(dsname, llq, out_extension, out_txtflag, out_ccsid) == 0) {
        strlower(out_extension);
        free(dsname);
        free(llq);
    ...
    if (get_configuration_defaults(dsname, llq, out_extension, out_txtflag, out_ccsid) == 0) {
        strlower(out_extension);
        free(dsname);
        free(llq);
        __ae_thread_setmode(prevMode);
...
int parse_configuration(cJSON *root, char* in_dsname, char* in_llq, char* out_extension, int *out_txtflag, int *out_ccsid)
{
  if (root == NULL) {
...
int get_user_configuration(char* in_dsname, char* in_llq, char* out_extension, int* out_txtflag, int* out_ccsid) {
    FILE *file = NULL;
    ...
    int rc = parse_configuration(root, in_dsname, in_llq, out_extension, out_txtflag, out_ccsid);
    cJSON_Delete(root);
...
int get_configuration_defaults(char* in_dsname, char* in_llq, char* out_extension, int* out_txtflag, int* out_ccsid) {
